...
export default {
	...
	run: async (bot, playerName: string) => {
		...
		const embed = new EmbedBuilder()
			...
			.setFooter({text:"made by @asumji",iconURL:"https://cdn.discordapp.com/avatars/612625159656046643/c6872f8a0ea475493e54746bb6e5b27b.webp?size=4096"})
				...
				res.data.profiles.forEach(async (profile: any) => {
					...
					if (isFetchError(lbData)) {
						...
						for (let lb in lbData) {
							lbs += `**#${lbData[lb]?.positionIndex}** ${lbData[lb]?.name.replace(/(?<=_|\b)[A-z]/g,(letter: string) => letter.toUpperCase()).replace(/_/g," ")}: \`${lbData[lb]?.value}\`\n`
						}
					}
					...
					let skillAvg = (getLevel(skills["SKILL_FARMING"] == undefined ? 0 : skills["SKILL_FARMING"],"farming",farmingCap) +
									getLevel(skills["SKILL_MINING"] == undefined ? 0 : skills["SKILL_MINING"],"mining") +
									getLevel(skills["SKILL_FORAGING"] == undefined ? 0 : skills["SKILL_FORAGING"],"foraging") +
									getLevel(skills["SKILL_COMBAT"] == undefined ? 0 : skills["SKILL_COMBAT"],"combat") +
									getLevel(skills["SKILL_ALCHEMY"] == undefined ? 0 : skills["SKILL_ALCHEMY"],"alchemy") +
									getLevel(skills["SKILL_CARPENTRY"] == undefined ? 0 : skills["SKILL_CARPENTRY"],"carpentry") +
									getLevel(skills["SKILL_FISHING"] == undefined ? 0 : skills["SKILL_FISHING"],"fishing") +
									getLevel(skills["SKILL_ENCHANTING"] == undefined ? 0 : skills["SKILL_ENCHANTING"],"enchanting") +
									getLevel(skills["SKILL_TAMING"] == undefined ? 0 : skills["SKILL_TAMING"],"taming",tamingCap))/9

					embed.addFields({name:"Vampire",value:`**Slayer XP**: ${vamp["xp"] != undefined ? vamp["xp"] : 0}\n**T4s**: ${vamp["boss_kills_tier_3"] != undefined ? vamp["boss_kills_tier_3"] : 0}\n**T5s**: ${vamp["boss_kills_tier_4"] != undefined ? vamp["boss_kills_tier_4"] : 0}`,inline:true},
									{name: "Skyblock",value:`**Level**: ${profile["members"][mojangProfile.id]["leveling"]["experience"]/100}\n**Skill Avg**: ${skillAvg.toFixed(2)}\n**Cata**: ${getLevel(profile["members"][mojangProfile.id]["dungeons"]["dungeon_types"]["catacombs"]["experience"],"dungeoneering")}`,inline:true},
									{name:"Top 100s",value:lbs == "" ? "None" : lbs})
					bot.sendGuildMessage("oc",`Vampire XP: ${vamp["xp"] != undefined ? vamp["xp"] : 0} T4s: ${vamp["boss_kills_tier_3"] != undefined ? vamp["boss_kills_tier_3"] : 0} T5s: ${vamp["boss_kills_tier_4"] != undefined ? vamp["boss_kills_tier_4"] : 0} Skyblock Level: ${profile["members"][mojangProfile.id]["leveling"]["experience"]/100}`)
					bot.officerChannel?.send({embeds:[embed]})
				})
			})
		}
